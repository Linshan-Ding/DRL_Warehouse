...
def train_ppo_agent(ppo_agent, warehouse, orders_test, num_episodes=10):
    ...
    for episode in range(num_episodes):
        ...
        # 计算订单平均拣选时间和按时完成率
        avg_picking_time = (sum([order.complete_time - order.arrive_time for order in test_env.orders_completed]) /
                            len(test_env.orders_completed))
        completion_rate = len([order for order in test_env.orders_completed if order.complete_time <= order.due_time]) / len(test_env.orders_arrived)
        print("订单平均拣选时间：", avg_picking_time)
        print("订单按时完成率：", completion_rate)
        # 存储三类数据到csv文件
        with open('instance_data_PPO_III.csv', 'a', newline='') as f:
            writer = csv.writer(f)
