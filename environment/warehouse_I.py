...
from environment.class_warehouse import PickPoint, StorageBin, Item, Depot, Picker, Robot
...
class WarehouseEnv(gym.Env, Config):
    def __init__(self):
        ...
        # 仓库环境参数
        self.N_l = self.parameter["shelf_capacity"]  # 单层货架中储货位的数量
    ...
    def state_extractor(self):
        ...
        # 拣货员总数
        n_pickers = len([picker for picker in self.pickers if picker.remove is False])
        # 每个区域的拣货员数量列表
    ...
    def idle_pickers(self):
        # 每个区域的空闲拣货员列表字典
        idle_pickers_area = {area_id: [picker for picker in self.pickers_area[area_id] if picker.state == 'idle']
                             for area_id in self.area_ids}
    ...
    def idle_pick_points(self):
        # 每个区域待分配拣货员的拣货位列表字典
        idle_pick_points_area = {area_id: [point for point in self.pick_points_area[area_id] if point.is_idle]
                                 for area_id in self.area_ids}
...
if __name__ == "__main__":
    ...
    for epoch in range(1000):
        ...
        # 仿真总时间一个月
        warehouse.reset(orders_object)  # 重置仓库环境
        total_reward = 0  # 总奖励
